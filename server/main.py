...
def get_video_results():
    ...
    for result in driver.find_elements("css selector",'.text-wrapper.style-scope.ytd-video-renderer'):
        ...
        try:
            time_published = result.find_element("css selector",'.style-scope ytd-video-meta-block').text.split('\n')[1]
        except:
            time_published = None

        try:
            snippet = result.find_element("css selector",'.metadata-snippet-container').text
        except:
            snippet = None

        try:
            if result.find_element("css selector",'#channel-name .ytd-badge-supported-renderer') is not None:
                verified_badge = True
            else:
        ...
        try:
            extensions = result.find_element("css selector",'#badges .ytd-badge-supported-renderer').text
        except:
            extensions = None
